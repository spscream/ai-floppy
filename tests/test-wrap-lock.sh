...
out1="$(cd "$repo" && bash "$ROOT/scripts/run" lock acquire "first" 2>&1)"; rc1=$?
assert_rc       "first acquire succeeds"            0 "$rc1"
assert_contains "first acquire reports ok"           "ok lock acquired" "$out1"

out2="$(cd "$repo" && bash "$ROOT/scripts/run" lock acquire "second" 2>&1)"; rc2=$?
...
out3="$(cd "$repo" && bash "$ROOT/scripts/run" lock status 2>&1)"; rc3=$?
...
out4="$(cd "$repo" && bash "$ROOT/scripts/run" lock release 2>&1)"
...
out5="$(cd "$repo" && bash "$ROOT/scripts/run" lock acquire "stale-owner" 2>&1)"
...
out6="$(cd "$repo" && bash "$ROOT/scripts/run" lock acquire "new-owner" 2>&1)"; rc6=$?
...
out7="$(cd "$repo" && bash "$ROOT/scripts/run" lock status 2>&1)"
assert_contains "status after takeover names the new owner" "owner=new-owner" "$out7"

out8="$(cd "$repo" && bash "$ROOT/scripts/run" lock status 2>&1)"
...
outS1="$(cd "$repoS" && bash "$ROOT/scripts/run" lock acquire "session A" 2>&1)"; rcS1=$?
...
outS2="$(cd "$wtS" && bash "$ROOT/scripts/run" lock acquire "session B" 2>&1)"; rcS2=$?
...
outO="$(cd "$repoO" && bash "$ROOT/scripts/run" lock acquire "other project" 2>&1)"; rcO=$?
...
outR="$(cd "$wtS" && bash "$ROOT/scripts/run" lock release 2>&1)"
