...
assert_eq "and not in the store checkout" "1" \
  "$([[ -f "$s_dir/private/projects/acme/note.md" ]] && echo 0 || echo 1)"
...
assert_eq "the note lands in the adopted checkout" "0" \
  "$([[ -f "$H3/agents_memory/private/projects/acme/note.md" ]] && echo 0 || echo 1)"
...
mkdir -p "$clone7/private/projects/acme"
mv "$clone7/projects/acme/note.md" "$clone7/private/projects/acme/note.md"
run_verb "$repo7" "$H7" workplace
...
mkdir -p "$W7/other/private/projects/acme"
rm -f "$repo7/.agent-memory/private"
ln -s "$W7/other/private/projects/acme" "$repo7/.agent-memory/private"
run_verb "$repo7" "$H7" workplace
