...
out="$(cd "$repo" && AI_FLOPPY_HOME="$ROOT" bash .floppy/run heat good-note "two words" 2>&1)"; rc=$?
...
out="$(cd "$repo" && AI_FLOPPY_HOME="$ROOT" bash .floppy/run heat --help 2>&1)"; rc=$?
...
repo_rf="$(sandbox)"; cp shim/run "$repo_rf/.floppy/run"
mkdir "$repo_rf/.floppy/heat.log.tmp"   # a directory: the trim's redirect fails
i=0
while [[ $i -lt 5100 ]]; do printf '2026-01-01 filler-%d\n' "$i"; i=$((i+1)); done >> "$repo_rf/.floppy/heat.log"
out="$(cd "$repo_rf" && AI_FLOPPY_HOME="$ROOT" bash .floppy/run heat my-note 2>&1)"; rc=$?
